...
bool copyDirectoryMissingFiles(const QString &sourcePath,
...
void AppDataPaths::migrateLegacyData()
{
    ...
    for (const QString &legacyRoot : legacyRoots) {
        ...
        copyDirectoryMissingFiles(
            QDir(legacyRoot).filePath("face_samples"),
            faceSamplesDirectory());
        copyDirectoryMissingFiles(
            QDir(legacyRoot).filePath("recognition_snapshots"),
            recognitionSnapshotsDirectory());
    }
}
