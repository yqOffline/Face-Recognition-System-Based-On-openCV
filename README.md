...
2. 安装 OpenCV 4.10.0。
3. 修改 `CMakeLists.txt` 中的 `OpenCV_DIR` 和 `OpenCV_DLL_DIR`，使其与本机安装位置一致。
4. 使用 Qt Creator 打开根目录的 `CMakeLists.txt`。
5. 选择 `Desktop Qt 6.11.1 MSVC2022 64bit` Kit。
...
- 当前项目没有账号、角色和管理员权限控制。
- `OpenCV_DIR` 仍是开发机绝对路径，后续可改为 CMake 缓存变量或环境变量，提高可移植性。
