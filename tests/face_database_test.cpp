...
int main(int argc, char *argv[])
{
    ...
    const QString previewPath = FaceSampleStorage::saveAlignedFace(
        previewImage, &storageError);
    ...
    if (!database.registerFaceSample("S001", "Alice", "Computer Science",
                                     firstFeature, previewPath, &personId, &created)
        || !created || personId < 0) {
    ...
    if (!database.registerFaceSample("S001", "Ignored", "Ignored",
                                     secondFeature, QString(), nullptr, &created)
        || created) {
    ...
    const QString snapshotPath = RecognitionLogStorage::saveSnapshot(
        previewImage, cv::Rect(20, 20, 60, 60), &storageError);
    if (snapshotPath.isEmpty()
        || !database.addRecognitionLog(personId, "Alice Zhang", 0.82f,
                                       snapshotPath, "Camera")) {
        std::cerr << "Failed to save recognition history: "
                  << database.lastError().toStdString() << '\n';
        return 12;
    }
    QVector<RecognitionLogInfo> logs = database.getRecentRecognitionLogs();
    if (logs.size() != 1 || logs.front().personId != personId
        || logs.front().nameSnapshot != "Alice Zhang"
        || std::abs(logs.front().similarity - 0.82f) > 0.001f) {
